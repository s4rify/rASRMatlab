function [] = peakToPeakMapVEP(p, channels,toposcale, extraInfo, method)
%
% peakToPeakMapVEP.m--
%
% Input arguments: 
...
% Sarah Blum (sarah.blum@uol.de), 2019-01-20 13:18
%-------------------------------------------------------------------------
 ...
 %
    ...
    for i = 1: length(flist)
        ...
        MEANTOPO1(i,:) = mean(mean(MERGED.data(:,[IND(1):IND(2)],:),2),3);
        MEANTOPO2(i,:) = mean(mean(MERGED.data(:,[IND(1):IND(2)],:),2),3);
   ...
   %method = strsplit(p, '/');
    supertitle(['Grand average VEP, ', method, ' , mean ntrials per subj: ', num2str(round(ntrials)),', ', extraInfo]);

    % TOPOs
    subplot(rows,columns, [1]);
    topoplot(mean(MEANTOPO1), MERGED.chanlocs, 'maplimits', toposcale, 'emarker2', {[channels], '.', 'w'});
    title('Peak to Peak map')
...
%     end
    ...
    set(fig, 'Color', [1,1,1]);
    set(gca, 'FontSize', 18);
    saveas(fig,['Figures/VEP/', [method, '_peakToPeak', extraInfo] ,'.fig']);